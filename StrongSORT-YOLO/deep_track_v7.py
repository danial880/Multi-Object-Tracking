...
def detect(save_img=False, line_thickness=1):
    ...
    for i in range(nr_sources):
        strongsort_list.append(
            DeepSort(model_path="weights/deepsort_pytorch/ckpt.t7", 
                max_dist=0.2, min_confidence=0.25, 
                nms_max_overlap=0.45, max_iou_distance=0.7, 
    ...
    for frame_idx, (path, img, im0s, vid_cap) in enumerate(dataset):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                outputs[i], detections = strongsort_list[i].update(xywhs.cpu(), confs.cpu(), im0)
                ...
                if len(outputs[i]) > 0:
                    for j, (output, conf) in enumerate(zip(outputs[i], confs)):
                        ...
                        if save_img:#vid or save_crop or show_vid :  # Add bbox to image
